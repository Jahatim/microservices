...
public class AccountsController {
    ...
    @PostMapping("/create")
    public ResponseEntity<ResponseDto> createAccount(@RequestHeader(name = "timbank_traceid") String traceId, @Valid @RequestBody CustomerDto customerDto) {
        LOGGER.info("Inside createAccount, traceId:{}", traceId);
    ...
    @GetMapping("/fetch")
    public ResponseEntity<CustomerDto> fetchAccountDetails(@RequestHeader(name = "timbank_traceid") String traceId, @RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits") String mobileNumber) {
        LOGGER.info("Inside fetchAccountDetails, traceId:{}", traceId);
    ...
    @PutMapping("/update")
    public ResponseEntity<ResponseDto> updateAccountDetails(@RequestHeader(name = "timbank_traceid") String traceId, @Valid @RequestBody CustomerDto customerDto) {
        LOGGER.info("Inside updateAccountDetails, traceId:{}", traceId);
    ...
    public ResponseEntity<ResponseDto> deleteAccountDetails(@RequestHeader(name = "timbank_traceid") String traceId, @RequestParam @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits") String mobileNumber) {
        LOGGER.info("Inside deleteAccountDetails, traceId:{}", traceId);
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "HTTP Status OK"), @ApiResponse(responseCode = "500", description = "HTTP Status Internal Server Error", content = @Content(schema = @Schema(implementation = ErrorResponseDto.class)))})
    @Retry(name = "javaVersionRetry", fallbackMethod = "getJavaVersionFallback")
    ...
    public ResponseEntity<String> getJavaVersionFallback(Throwable throwable) {
        return ResponseEntity.status(HttpStatus.OK).body("Java Version is not available");
    }
